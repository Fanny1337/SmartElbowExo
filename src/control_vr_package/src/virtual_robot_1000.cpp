...
//     {
//         double dt = 1.0; //Hz kanske kan fungera även om man minskar denna?
...
//         double g_0 = 9.81, Beta = 0.5; //bode denna vara 0.5 eller 0.1?
...
public:
    DynamicModelNode() : Node("dynamic_model")
    {
        RCLCPP_INFO(this->get_logger(), "Starting Dynamic Model Node...");
        ...
        RCLCPP_INFO(this->get_logger(), "Subscribed to elbow_torque and publishing to elbow_position_cmd");
...
private:
    void torque_callback(const std_msgs::msg::Float64::SharedPtr msg)
    {
        RCLCPP_INFO(this->get_logger(), "Received torque: %f", msg->data);
        double dt = 1.0;
        auto new_state = rk4_integration(current_state_, msg->data, dt);
        current_state_ = new_state;
        RCLCPP_INFO(this->get_logger(), "Hej------------------------: %f", current_state_[0]);
        publish_state();
    }
    ...
    void publish_state()
    {
        ...
        position_msg.data = current_state_[0];
        position_publisher_->publish(position_msg);
        RCLCPP_INFO(this->get_logger(), "Published new state position: %f", current_state_[0]);
    }
